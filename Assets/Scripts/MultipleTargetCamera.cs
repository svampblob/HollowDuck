using System.Collections;
using System.Collections.Generic;
using UnityEngine;

[RequireComponent(typeof(Camera))]
public class MultipleTargetCamera : MonoBehaviour
{

    public List<Transform> targets;
    public Vector3 offset;
    public float smoothTime = 0.2f;
    public float minZoom = 6f;
    public float maxZoom = 2f;
    public float zoomLimiter = 10f;
    private Vector3 velocity;
    private Camera cam;
    public bool one;
    public bool two;
    public bool three;
    public bool four;

    PlayersInGame playersInGame;

    public Camera maincam;
    public float shakeAmount = 0;

    void Start()
    {
        targets.Clear();
        cam = GetComponent<Camera>();
        Targets();
    }

   void Targets()
    {

        if (GameObject.FindGameObjectWithTag("Player1").transform != null && one == false)
        {
            print("Hello");
            targets.Add(GameObject.FindGameObjectWithTag("Player1").transform);
            one = true;
        }
        if (GameObject.FindGameObjectWithTag("Player2").transform != null && two == false)
        {
            targets.Add(GameObject.FindGameObjectWithTag("Player2").transform);
            two = true;
        }
        if (playersInGame.Player3 != null)
        {
            print("Player3 found");
            targets.Add(playersInGame.Player3.transform);
            three = true;
        }
        if (GameObject.FindGameObjectWithTag("Player4").transform != null && four == false)
        {
            targets.Add(GameObject.FindGameObjectWithTag("Player4").transform);
            four = true;
        }
    }
    private void Update()
    {
        print("players active" + targets.Count);
    }

    private void LateUpdate()
    {
        if (targets.Count == 0)
            return;

        Move();
        Zoom();
    }


    void Zoom()
    {
        float newZoom = Mathf.Lerp(maxZoom, minZoom, GetGreatestDistance() / zoomLimiter);
        // print (GetGreatestDistance() / zoomLimiter);
        cam.orthographicSize = Mathf.Lerp(cam.orthographicSize, newZoom, Time.deltaTime);
        Vector2 Left = targets[0].transform.position;
        Vector2 Right = targets[0].transform.position;
        if (targets.Count > 1)
        {

            for (int i = 0; i < targets.Count; i++)
            {
                if (targets[i].transform.position.x < Left.x)
                {
                    Left = targets[i].transform.position;
                }
                if (targets[i].transform.position.x > Right.x)
                {
                    Right = targets[i].transform.position;
                }
            }

        }
        float dist = Vector2.Distance(Left, Right);
    }
    void Move()
    {
        Vector3 centerPoint = GetCenterPoint();

        Vector3 newPosition = centerPoint + offset;

        transform.position = Vector3.SmoothDamp(transform.position, newPosition, ref velocity, smoothTime);
    }
    float GetGreatestDistance()
    {
        var bounds = new Bounds(targets[0].position, Vector3.zero);
        for (int i = 0; i < targets.Count; i++)
        {
            bounds.Encapsulate(targets[i].position);
        }
        return bounds.size.x;
    }

    Vector3 GetCenterPoint()
    {
        if (targets.Count == 1)
        {
            return targets[0].position;
        }


        var bounds = new Bounds(targets[0].position, Vector3.zero);

        for (int i = 0; i < targets.Count; i++)
        {


            bounds.Encapsulate(targets[i].position);



        }

        return bounds.center;



    }
    void Awake()
    {
        if (maincam == null)
        {
            maincam = cam;
        }
    }

    public void shake(float amt, float lenght)
    {
        shakeAmount = amt;
        InvokeRepeating("BeginShake", 0, 0.01f);
        Invoke("StopShake", lenght);
    }

    void BeginShake()
    {
        if (shakeAmount > 0)
        {
            Vector3 campos = cam.transform.position;

            float offsetX = Random.value * shakeAmount * 2 - shakeAmount;
            float offsetY = Random.value * shakeAmount * 2 - shakeAmount;
            campos.x += offsetX;
            campos.y += offsetY;

            cam.transform.position = campos;

        }
    }

    void StopShake()
    {
        CancelInvoke("BeginShake");
    }
}
